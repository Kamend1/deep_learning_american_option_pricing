    ...
    *,
    ...
    for batch in loader:
        ...
        totals["residual_absolute_error"] += float(
            torch.abs(predicted_residual.detach() - residual_target).sum().cpu()
        )
        predicted_label = (torch.sigmoid(exercise_logits.detach()) >= 0.5).float()
    ...
    return {
        ...
        "classification_accuracy": totals["classification_correct"] / observations,
...
def fit_multitask_model(
    ...
    *,
    ...
    """Train with validation scheduling, early stopping, and atomic checkpointing."""
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        row = {
            ...
            "validation_classification_accuracy": validation_metrics["classification_accuracy"],
            ...
            atomic_torch_save(
                {
                    ...
                    "positive_class_weight": float(loss_fn.positive_class_weight.item()),
                },
                checkpoint,
            )
        else:
...
def predict_multitask_model(
    ...
    *,
    ...
    for batch in loader:
        ...
        normalized_floor = torch.maximum(normalized_european, normalized_intrinsic)
            ...
            pd.DataFrame(
                {
                    ...
                    "predicted_residual": predicted_residual.cpu().numpy().reshape(-1),
                    "predicted_normalized_american_price": normalized_price.cpu().numpy().reshape(-1),
                    "exercise_logit": exercise_logits.cpu().numpy().reshape(-1),
                    "exercise_probability": probability.cpu().numpy().reshape(-1),
                    "exercise_prediction": (
...
def run_classifier_epoch(
    ...
    *,
    ...
    """Run one exercise-only classifier epoch using multi-task loaders."""
    ...
    for batch in loader:
        ...
        correct += float((predicted == target).sum().cpu())
...
def fit_exercise_classifier(
    ...
    *,
    ...
    """Train the exercise-only neural baseline."""
    ...
    for batch in loader:
        ...
        rows.append(
            pd.DataFrame(
                {
                    "sample_id": np.asarray(row_ids),
                    "exercise_logit": logits.cpu().numpy().reshape(-1),
                    "exercise_probability": probability.cpu().numpy().reshape(-1),
                }
            )
        )
    return pd.concat(rows, ignore_index=True) if rows else pd.DataFrame(
        columns=["sample_id", "exercise_logit", "exercise_probability"]
    )
