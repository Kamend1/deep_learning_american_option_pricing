    ...
    for batch in batches:
        ...
        states[batch.contract.contract_id] = _BackwardState(
            ...
            exercise_indices=np.full(
                batch.n_paths, batch.n_steps, dtype=np.int32
            ),
        )
    return states
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        with torch.no_grad():
            ...
            validation_loss = float(
                loss_fn(validation_predictions, val_y.to(device)).item()
            )
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, len(transformed), batch_size):
            tensor = torch.from_numpy(transformed[start : start + batch_size]).to(device)
    ...
    *,
    ...
    for batch in batches:
        ...
        continuation = _predict_normalized_continuation(
            model, standardizer, features, device=device
        ) * contract.strike
...
def fit_neural_lsm_policy(
    ...
    *,
    ...
    if _validate_batch_collection(validation_batches) != n_steps:
        raise ValueError("Training and validation batches need equal step counts.")
    ...
    for step_index in range(n_steps - 1, 0, -1):
        train_x, train_y = _collect_step_samples(
            training_batches, training_states, step_index=step_index
        )
        val_x, val_y = _collect_step_samples(
            validation_batches, validation_states, step_index=step_index
        )
        ...
        initial_state = (
            later_state if config.warm_start_from_later_step else None
        )
    ...
    return NeuralLSMPolicy(n_steps=n_steps, steps=policy_steps), pd.DataFrame(history_rows)
...
def evaluate_neural_lsm_policy(
    ...
    *,
    ...
    payoffs = np.maximum(contract.strike - batch.paths[:, -1], 0.0)
    ...
    for step_index in range(1, policy.n_steps):
        ...
        features = build_continuation_features(
            spots[itm], contract=contract, time_remaining=time_remaining
        )
        ...
        if exercise_path_indices.size:
            payoffs[exercise_path_indices] = np.maximum(
                contract.strike - batch.paths[exercise_path_indices, step_index],
                0.0,
            )
